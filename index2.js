...
dashboardBtnActivities.addEventListener('click',async() => {
...
dashboardContainer.addEventListener('mouseleave',async() =>{//cerrando lista cuando mouse no este ensima
    await setTimeout(()=>{
    dashboardActivities.style.display = 'none';
     dashboardBtnActivities.classList.replace('bx-caret-left', 'bx-caret-right');
    dashboardBtnPoints.style.display = 'block';
...
//desplegando actividad MEDIANA
const btnMediana = document.querySelector('.bx-math');
const mediana = document.getElementById('mediana');
document.get
btnMediana.addEventListener('click', () =>{
    mediana.style.display = 'block';
    figuras.style.display = 'none';
    nomina.style.display = 'none';
})
...
function  comparacion(argument){ document.getElementById('titlNomActualComp').style.display = 'inline-block';
argument.style.display = 'block';

 argument == document.formNomActualComp?(document.getElementById('titlNomActual').style.visibility = 'none',
